# Launches the Flutter Android app in Firebase auth preview mode.
# Reads secrets from firebase_android.local.ps1 (git-ignored, not committed).
# Setup: copy firebase_android.local.ps1.example to firebase_android.local.ps1 and fill in real values.
# Usage: .\run_android_firebase.ps1 [deviceId]  (defaults to first available Android device)
...
try {
    $deviceArg = if ($DeviceId) { $DeviceId } else { 'android' }
    flutter run -d $deviceArg `
